        ...
        if login_user:
            if bcrypt.hashpw(request.form['pass'].encode('utf-8'), login_user['password'])==login_user['password']:
                ...
                return render_template("index.html", username=session['username'],
                        recipes=mongo.db.recipes.find(),courses=mongo.db.meal_courses.find(), title='Welcome '+session['username'])
...
def register():
    if request.method == 'POST':
        ...
        if existing_user is None:
            ...
            users.insert_one({'username' : request.form['username'], 'password' : hashpass, 'email' : request.form['email'], 'role': 'user'})
...
def show_one_recipe(recipe_id):
    ...
    return render_template("show_one_recipe.html", recipes=the_recipe, cook_name=cook_name)
...
def show_recipes():
    ...
    return render_template("show_recipes.html", recipes=recipes, title='Browse recipes')
